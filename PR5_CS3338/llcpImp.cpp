...
void RemOddDupEven(Node*& headPtr)
{
    ...
    // Check for even and odd valued elements
    while (*cursor)
    {
        // If even element is found, create new node and duplicate the even value
        if (!((*cursor)->data % 2))
        {
            Node *n = new Node;                 // Create new node
            n->data = (*cursor)->data;          // Copy current data
            n->link = (*cursor)->link;          // Copy current link
            (*cursor)->link = n;                // Insert new node after current
            cursor = &(*cursor)->link->link;    // Increment to correct node
        }
        // If odd element is found, correct list and delete the node
        else
        {
            Node *temp = *cursor;     // Keep track of current.
            *cursor = temp->link;     // Move past current.
            delete temp;              // Remove odd node.
        }
    }
}
